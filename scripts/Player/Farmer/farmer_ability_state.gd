class_name FarmerAbilityState
extends BaseState

var corpse_found = false
var hit_enemies_set = []

var egg_scene = preload("res://Scenes/chicken_egg.tscn")
	...
	if !corpse_found:
		for body in player_root.get_tree().get_nodes_in_group("Corpse"):
			if player_root.global_position.distance_to(body.global_position) < 30:
				Global.egg_positions.append(body.global_position)
				var egg = egg_scene.instantiate()
				player_root.get_tree().current_scene.add_child(egg)
				egg.global_position = body.global_position
				body.interact()
				corpse_found = true
				print(Global.egg_positions)
